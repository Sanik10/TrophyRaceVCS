...
static public class GameObjectTypeLoggingSM
{
    ...
    static public void LogStageInformation(GameObject go)
    {
        ...
        else
        {
            ...
            if (prefabStage != null)
            {
                ...
                {
                    //var prefabAssetRoot = AssetDatabase.LoadAssetAtPath<GameObject>(prefabStage.prefabAssetPath);
                    var prefabAssetRoot = AssetDatabase.LoadAssetAtPath<GameObject>(prefabStage.assetPath);
                    var type = PrefabUtility.GetPrefabAssetType(prefabAssetRoot);
                    Debug.Log(string.Format("GameObject is in a PrefabStage. The opened Prefab is of type: {0}. The GameObject comes from the prefab asset: {1}", type,prefabStage.assetPath));// prefabStage.prefabAssetPath));
                }
            }
            else if (EditorSceneManager.IsPreviewSceneObject(go))
            {
    ...
    static public void LogPrefabInformation(GameObject go)
    {
        ...
        if (prefabStage != null)
        {
            GameObject openPrefabThatContentsIsPartOf = AssetDatabase.LoadAssetAtPath<GameObject>(prefabStage.assetPath);//(prefabStage.prefabAssetPath);
