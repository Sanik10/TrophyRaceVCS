	...
	public class RealtimeReflection : MonoBehaviour
	{
		...
		void Awake()
		{
			...
			if (reflectionProbe == null) {
				if (transform.Find("Real time reflection") && transform.Find("Real time reflection").GetComponent <ReflectionProbe>()) {
					reflectionProbe = transform.Find("Pro Car Paint Reflection").GetComponent <ReflectionProbe>();
				} else {
					reflectionProbe = new GameObject("Real time reflection").AddComponent <ReflectionProbe>();
					this.setSettings();
				}
			}
		}
