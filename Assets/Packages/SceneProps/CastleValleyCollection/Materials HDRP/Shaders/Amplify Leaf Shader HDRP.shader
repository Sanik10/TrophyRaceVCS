...
Shader "Amplify Leaf Shader HDRP"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_ColorTexture("Color Texture", 2D) = "white" {}
		...
		[ASEEnd]_MovementMultiplier("Movement Multiplier", Float) = 0.07
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float mulTime18 = _TimeParameters.x * _WindFrequency;
				float2 temp_cast_0 = (mulTime18).xx;
				float2 texCoord20 = inputMesh.ase_texcoord.xy * float2( 1,1 ) + temp_cast_0;
				float simplePerlin2D21 = snoise( texCoord20*_NoiseScale );
				simplePerlin2D21 = simplePerlin2D21*0.5 + 0.5;
				float temp_output_26_0 = ( ( -0.5 + simplePerlin2D21 ) * _MovementMultiplier );
				float simplePerlin2D22 = snoise( texCoord20*_NoiseScale );
				simplePerlin2D22 = simplePerlin2D22*0.5 + 0.5;
				float temp_output_27_0 = ( _MovementMultiplier * ( -0.5 + simplePerlin2D22 ) );
				float3 appendResult30 = (float3(temp_output_26_0 , temp_output_27_0 , ( ( temp_output_26_0 + temp_output_27_0 ) * 0.4 )));
				...
				#endif
				float3 vertexValue = ( inputMesh.uv1.y * appendResult30 );
						...
						)
			{
				...
				float2 uv_ColorTexture = packedInput.ase_texcoord5.xy * _ColorTexture_ST.xy + _ColorTexture_ST.zw;
				float4 tex2DNode12 = tex2D( _ColorTexture, uv_ColorTexture );
				...
				float4 tex2DNode14 = tex2D( _MaskTexture, uv_MaskTexture );
				
				surfaceDescription.Albedo = tex2DNode12.rgb;
				surfaceDescription.Normal = UnpackNormalScale( tex2D( _NormalTexture, uv_NormalTexture ), 1.0f );
				...
				surfaceDescription.Occlusion = tex2DNode14.g;
				...
				#if defined(_HAS_REFRACTION) || defined(_MATERIAL_FEATURE_TRANSMISSION)
				surfaceDescription.Thickness = tex2DNode14.b;
				#endif
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float mulTime18 = _TimeParameters.x * _WindFrequency;
				float2 temp_cast_0 = (mulTime18).xx;
				float2 texCoord20 = inputMesh.uv0.xy * float2( 1,1 ) + temp_cast_0;
				float simplePerlin2D21 = snoise( texCoord20*_NoiseScale );
				simplePerlin2D21 = simplePerlin2D21*0.5 + 0.5;
				float temp_output_26_0 = ( ( -0.5 + simplePerlin2D21 ) * _MovementMultiplier );
				float simplePerlin2D22 = snoise( texCoord20*_NoiseScale );
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_NormalTexture = packedInput.ase_texcoord.xy * _NormalTexture_ST.xy + _NormalTexture_ST.zw;
				
				float2 uv_MaskTexture = packedInput.ase_texcoord.xy * _MaskTexture_ST.xy + _MaskTexture_ST.zw;
				float4 tex2DNode14 = tex2D( _MaskTexture, uv_MaskTexture );
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float3 defaultVertexValue = float3( 0, 0, 0 );
				#endif
				float3 vertexValue = ( inputMesh.ase_texcoord1.y * appendResult30 );
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_ColorTexture = packedInput.ase_texcoord3.xy * _ColorTexture_ST.xy + _ColorTexture_ST.zw;
				float4 tex2DNode12 = tex2D( _ColorTexture, uv_ColorTexture );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_ColorTexture = packedInput.ase_texcoord7.xy * _ColorTexture_ST.xy + _ColorTexture_ST.zw;
				float4 tex2DNode12 = tex2D( _ColorTexture, uv_ColorTexture );
...
WireConnection;42;7;53;0
WireConnection;42;8;14;2
WireConnection;42;9;12;4
WireConnection;42;16;14;3
WireConnection;42;21;16;0
WireConnection;42;11;32;0
ASEEND*/
//CHKSM=C5CFEAE7128C687AA52776E1E963495CAAFEEFC8